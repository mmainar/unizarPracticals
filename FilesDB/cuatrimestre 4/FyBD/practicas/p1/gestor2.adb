...
-- Versión: 1.0
-- Comentarios: Utiliza el modulo FCOTIZACIONESPG.
--------------------------------------------------------------------------
with ada.text_IO, ada.integer_text_IO, ada.float_text_IO, FCOTIZACIONESPG;
use ada.text_IO, ada.integer_text_IO, ada.float_text_IO, FCOTIZACIONESPG;

procedure gestor2 is
 ...
 procedure generarFichCliente(f,g: in out tpFCotizacionesPG; cod: in tpTicker;
 ...
 procedure generarFichPeriodo(f,g: in out tpFCotizacionesPG; fecha1,fecha2:
 ...
 procedure comprobarCotizacion(f: in out tpFCotizacionesPG; fecha: in tpFecha;
                              cod: in tpTicker) is
  exito: boolean:= false;
  cot: cotizacion;
 begin
 ...
 procedure actualizarCotizacion(f: in out tpFCotizacionesPG; cot: in
                                                           cotizacion) is
  exito: boolean;
  aux: cotizacion;
  g: tpFCotizacionesPG; -- Fichero auxiliar para la copia
  pos, actual: integer;
  fichAux: tpNomFich;
 begin
  ...
  if exito then
    ...
    leerCotizacion(f,aux);
    asignarVentana(g,aux); poner(g); escribirBloque(g);
    iniciarLectura(g); iniciarEscritura(f);
    while not finFichero(g) loop
      leerCotizacion(g,aux);
      escribirCotizacion(f,aux);
    end loop;
    cotizacionVentana(g,aux); asignarVentana(f,aux); poner(f);
    escribirBloque(f); disociar(g);
 ...
 procedure insertarCotizacion(f: in out tpFCotizacionesPG; cot: in cotizacion) is
  exito: boolean:= false;
  aux: cotizacion;
  g: tpFCotizacionesPG; -- Fichero auxiliar para la copia
  pos: integer;
 ...
 procedure realizarOperacion(opcion: in natural; f: in out tpFCotizacionesPG;
