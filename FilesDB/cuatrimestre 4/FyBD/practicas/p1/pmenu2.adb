...
-- Versión: 1.0
--------------------------------------------------------------------------
with ada.text_IO, ada.integer_text_IO, FCOTIZACIONESPG;

use ada.text_IO, ada.integer_text_IO, FCOTIZACIONESPG;
...
procedure pmenu2 is
 ...
 procedure asociarFichero(f: in out tpFCotizacionesPG;
 ...
 procedure escribirCotizacionFichero (f: in out tpFCotizacionesPG;
                                     c: cotizacion) is
 ...
 begin
  ...
  put("La cotizacion escrita es : "); mostrarCotizacion(c); new_line;
 end escribirCotizacionFichero;


 procedure buscarFechaFichero (f: in out tpFCotizacionesPG;
 ...
 procedure buscarCotizacionFichero (f: in out tpFCotizacionesPG;
                                   c: out cotizacion;
                                   exito: out boolean) is
 ...
 procedure as (f: in out tpFCotizacionesPG; control: in out tpControlFich) is
 ...
 procedure ds (f: in out tpFCotizacionesPG; control: in out tpControlFich) is
 ...
 procedure il (f: in out tpFCotizacionesPG; control: in out tpControlFich;
              c: out cotizacion) is
 -- Realiza las operaciones necesarias para iniciar la lectura de un fichero.
 begin
 put("Iniciando lectura..."); new_line;
 if control.modo=E then escribirBloque(f); end if;
 iniciarLectura(f); control.recienAbierto:= true;
 cotizacionVentana(f,c);
 control.modo:= L;
 end il;


 procedure ie (f: in out tpFCotizacionesPG; control: in out tpControlFich) is
 ...
 procedure lc (f: in out tpFCotizacionesPG; control: in out tpControlFich;
              c: out cotizacion) is
  ...
  case control.modo is
   ...
   when L => if finFichero(f)
                then put("No hay mas cotizaciones por leer.");
                  ...
                  put("La cotizacion leida es: " ); mostrarCotizacion(c);
                  new_line;
             end if;
    when others => null;
  end case;
 ...
 procedure ec (f: in out tpFCotizacionesPG; control: tpControlFich;
              c: cotizacion) is
 ...
 procedure ff (f: in out tpFCotizacionesPG; control: tpControlFich) is
 ...
 procedure bf (f: in out tpFCotizacionesPG; control: tpControlFich;
              c: out cotizacion) is
 ...
 procedure bc (f: in out tpFCotizacionesPG; control: tpControlFich;
              c: out cotizacion) is
 ...
 procedure tratarOpcion (cad: in string; f: in out tpFCotizacionesPG;
