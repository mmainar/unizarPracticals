...
Package body reconocimiento is
    ...
    function restr_unarias(p: Par; ep: float; Obser, Modelo: Rectas) return boolean is
    ...
    function restr_binarias(pp: integer; p: par; ep: float; Sh: Pares; Obser, Modelo: Rectas) return boolean is
    ...
    procedure busca_pares(Sh: Pares; indiceObser: integer; Obser, Modelo: Rectas; Resul: out Soluciones; ep: float) is
    ...
    begin
	  ...
	  p := (indiceObser, m);
	  if (restr_unarias(p, ep, Obser, Modelo)) then
	    aceptar := true;
	    for pp in 1..Sh.n loop
	      aceptar := restr_binarias(pp, p, ep, Sh, Obser, Modelo);
              exit when not aceptar;	      
	    end loop;
	    if aceptar then
	      Sh2 := Sh;
	      Anadir(Sh2, p);
              busca_pares(Sh2, indiceObser+1, Obser, Modelo, Resul, ep);
	    end if;
	  end if;    
        end loop;
  ...
  -- Devuelve todas las soluciones posibles, ordenadas por numero de pares creciente    
    ...
    Sh: pares;
    Obser2: Rectas(Obser.n);
  begin -- de Reconocer
    busca_pares(Sh, 1, Obser, Modelo, Resul, 0.1);       
    -- Eliminar de la escena las observaciones procedentes del objeto ya reconocido
    for i in 1..Obser.n loop
      Obser2.elem(i) := Obser.elem(i);
    end loop;
    -- Continuar la busqueda de mas objetos en la escena
    busca_pares(Sh, 1, Obser2, Modelo, Resul, 0.1);
  end;
