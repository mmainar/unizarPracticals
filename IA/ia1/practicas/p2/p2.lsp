...
(defun combina-todas (xlist ylist) 
  "Devuelve una lista de listas formada a?endo una y a una x. 
  P.en, (combina-todas '((a) (b)) '((1) (2))) 
  -> ((A 1) (B 1) (A 2) (B 2))." 
   (mapcan #'(lambda (y) 
...
(defun genera-mio (frase) 
  "Genera una sentencia o frase aleatoria." 
  (let ((posibilidades (sustituciones frase)))
  (cond ((listp frase)               ;caso de lista 
         (mapcan #'genera-mio frase)) 
        (posibilidades  ;caso de simbolo con sustituciones 
         (genera-mio (random-elt posibilidades))) 
        (t (list frase)))))           ;caso de palabra 


(defun quita-palabra (palabra)
  "Quita una palabra de la gramatica de forma no destructiva."
  (let ((nueva-gramatica nil))
    (dolist (elem *gramatica*)
      (print (setq nueva-gramatica (append nueva-gramatica (list (remove palabra elem))))))
    nueva-gramatica))
...
(defun quita-palabra (palabra)
  ...
  (let ((nueva-gramatica nil))
    (dolist (elem *gramatica*)
      (push (remove palabra elem) nueva-gramatica ))
    nueva-gramatica))

(defun quita-palabra2 (palabra)
  "Quita una palabra de la gramatica de forma destructiva."
  (let ((nueva-gramatica nil))
    (dolist (elem *gramatica*)
      (push (delete palabra elem) nueva-gramatica ))
    nueva-gramatica))
...
(defparameter *gramatica-mia* 
  '((Sentencia -> (Nombre-frase Verbo-frase)) 
    (Nombre-frase -> (Articulo Nombre)) 
    (Verbo-frase -> (Verbo Nombre-frase)) 
    (Articulo -> uno el) 
    (Nombre -> hombre balon mujer mesa) 
    (Verbo -> golpeo cogio vio gusto)) 
  "Gramatica para un subconjunto trivial de Español.") 

(setq *gramatica* *gramatica-mia*)