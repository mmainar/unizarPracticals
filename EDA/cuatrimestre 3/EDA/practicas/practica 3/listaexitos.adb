...
package body listaexitos is
  ...
  procedure listado(l: in lista) is
  -- Post: lista en pantalla las canciones almacenadas en la lista l
    aux: ptDato;
  begin
    new_line; aux:= ptDato(l);
    put("Titulo                       Interprete                    Fecha         Ventas");
    new_line;
    put("-------------------------------------------------------------------------------");
    new_line;
    while (aux/=null) loop
      put(aux.dato.titulo);
      for i in 1..27-length(aux.dato.titulo) loop
        put(' ');
      end loop;
      put("  "); put(aux.dato.interprete);
      for i in 1..28-length(aux.dato.interprete) loop
        put(' ');
      end loop;
      put("  ");
      put(eldia(aux.dato.tiempo),2); put('/');
      put(elmes(aux.dato.tiempo),2); put('/');
      put(elanyo(aux.dato.tiempo),4); put("  ");
      put(aux.dato.ventas,8);
      aux:=aux.sig; new_line;
    end loop;
  end listado;
  ...
  -- Solicita el nombre de un fichero de texto y almacena en el la
  -- lista de éxitos l en su estado actual
    aux: ptDato;
  begin
